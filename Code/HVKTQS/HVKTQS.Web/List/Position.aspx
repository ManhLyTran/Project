<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Position.aspx.cs" Inherits="HVKTQS.Web.Position" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <div class="page-head">
        <div class="page-title break-head">
            <h1>Quản lý danh mục chức vụ</h1>
        </div>
    </div>
    <div class="row">
        <div class="col-md-8">
            <div class="portlet box green">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <asp:Label ID="lblTitle" runat="server" Text="Danh mục chức vụ"></asp:Label>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-md">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="dlgAddEdit-title">Thêm chức vụ</h4>
                </div>
                <div class="modal-body" style="max-height: 500px;">
                    <div class="form-horizontal">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <asp:TextBox ID="txtPositionName" CssClass="form-control input-xs" spellcheck="false" autocomplete="off" runat="server"></asp:TextBox>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        function loadList() {
            $.ajax({
                url: "../List/Handler/PositonHandler.ashx?fn=GetHTML",
                type: "POST",
                async: true,
        ...
        function updateTitle() {
            ...
            if (trs != null) {
                $('#<%=lblTitle.ClientID%>').text('Danh mục chức vụ (' + trs.length + ')');
            }
        }
        ...
        function clearValue() {
            ...
            $("#d-alert").html("");
            $("#d-alert").hide();
            $("#<%=txtPositionName.ClientID%>").focus();
        }

        function isValidated() {
            var strErr = "";

            if ($.trim($("#<%=txtPositionName.ClientID%>").val()) == "") {
                if (strErr == "") {
        ...
        function save() {
            var PositionName = $.trim($("#<%=txtPositionName.ClientID%>").val());
            var PositionID = parseInt($.trim($("#<%=hddRowId.ClientID%>").val()));
            var objInfo = {};
            objInfo.PositionName = PositionName;
            objInfo.PositionID = PositionID;
            $.ajax({
                url: "../List/Handler/PositonHandler.ashx?fn=AddUpdate",
                type: "POST",
                async: true,
        ...
        $(document).ready(function () {
            ...
            $('#btnSaveViewOrder').click(function (evt) {
                ...
                $trs.each(function (idx, item) {
                    strPositionID += $.trim($(item).find('td').eq(0).text()) + ",";
                });
                if (strPositionID == "") return false;

                $.ajax({
                    url: "../List/Handler/PositonHandler.ashx?fn=UpdateViewOrderInBatches",
                    type: "POST",
                    async: true,
                    data: "strPositionID=" + strPositionID,
                    success: function (response, textStatus, jqXHR) {
                        if (response.c > 0) {
                            alert(response.msg);
                            loadList();
            ...
            $("body").on("click", "#btnAdd", function (evt) {
                $("#dlgAddEdit-title").text("Thêm chức vụ");
            ...
            $("body").on("click", ".lnkEdit", function (evt) {
                $("#dlgAddEdit-title").text("Sửa chức vụ");
                $("#dlgAddEdit-btnSaveAndAdd").hide();
                var tr = $(this).closest("tr");
                var PositionID = $.trim($(tr).find('td').eq(0).text());
                $("#<%=hddRowId.ClientID%>").val(PositionID);

                $.ajax({
                    url: "../List/Handler/PositonHandler.ashx?fn=GetByIDCache",
                    type: "POST",
                    async: true,
                    data: "PositionID=" + PositionID,
                    success: function (response, textStatus, jqXHR) {
                        var objInfo = $.parseJSON(response.d);
                        $("#<%=txtPositionName.ClientID%>").val(objInfo.PositionName)
            ...
            $("body").on("click", ".lnkDelete", function (evt) {
                if (confirm("Bạn có chắc chắn muốn xóa không?") == true) {
                    ...
                    $.ajax({
                        url: "../List/Handler/PositonHandler.ashx?fn=DeleteByID",
                        type: "POST",
                        async: true,
                        data: "PositionID=" + PositionID,
                        success: function (response, textStatus, jqXHR) {
